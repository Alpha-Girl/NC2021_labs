              ...
              \item（10分）推导出使用Richardson外推技术的向前差商的计算公式并用伪代码 给出算法。
                    ...
                    \begin{breakablealgorithm}
                        ...
                        \begin{algorithmic}[1]
                            ...
                            \State $A\left[1\right]$ $\leftarrow$ $N_{1}\left(\frac{h}{2}\right)$;
                            \State $A\left[2\right]$ $\leftarrow$ $N_{1}\left(h\right)$;
                            \State $tmp$ $\leftarrow$ $2A\left[2\right]-A\left[1\right]$;
                            \While {$\left\lvert tmp-A\left[1\right]\right\rvert\ and\ A.length \leq maxrept $}
                            \State $A\left[2\right]$ $\leftarrow$ $tmp$;
